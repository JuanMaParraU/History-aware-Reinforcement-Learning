...
parser.add_argument('--action_space', default=['east', 'west', 'south', 'north', 'stay'], type=list,
...
def environment_setup():
    ...
    if args.numDrones == 2:
        ...
        dronePos = np.array(
            [[5, 5, 30], [95, 95, 30], [5, 95, 30], [95, 5, 30], [5, 50, 30], [50, 5, 30], [95, 50, 30], [50, 95, 30]])
...
def save_Q_table_mqtt(table, SINR, initial_real_reword, lambdaVar,action, dronePos, episode, step, drone,
                      topic_name = args.q_table_topic, host=args.mqttBroker, port=args.port):
...
def theoretical_performance(userPos, num):
    ...
    for i in range(num):
        ...
        drone_Pos[:, 2] = 30
        _, _, initial_real_reword = models.alloc_users(userPos, drone_Pos, args.fc, args.dAngle, args.N0, args.BW,
    ...
    for i in range(args.episode):
        ...
        for j in range(args.step):
            ...
            for k in range(args.numDrones):
                ...
                _, _, initial_real_reword = models.alloc_users(userPos, dronePos, args.fc, args.dAngle, args.N0,
                                                               args.BW, args.Pt, args.connectThresh)
                second_state = dronePos
                second_table_reword, action, Q_table[k] = Q.choose_max_action(dronePos[k][:2], Q_table[k])
                allocVec, SINR, second_real_reword = models.alloc_users(userPos, dronePos, args.fc, args.dAngle,
                                                                        args.N0, args.BW, args.Pt, args.connectThresh)
                dronePos = second_state % args.length
                if j <= args.step * 3 / 4 or j > args.step * 4 / 5:
                    Q_table[k] = Q.update_Q_table(Q_table[k], initial_state[k][:2], initial_action,
                                                  initial_table_reword, second_state[k][:2], second_table_reword,
                                                  second_real_reword['total'])
                rewords = initial_real_reword['total']
                #ßsave_Q_table_mqtt(Q_table, SINR, initial_real_reword, Lambda,action, dronePos, i, j, k)
            counter += 1
