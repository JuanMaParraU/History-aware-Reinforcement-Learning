...
parser.add_argument('--numUsers', default=100, type=int, help='The number of Users')
...
parser.add_argument('--episodes', default=20, type=int, help='The number turns it train')
parser.add_argument('--steps', default=2000, type=int, help='The number of steps for any turn of train')
...
#=======================================================================================================================
# Database Parameters
parser.add_argument('--database_name', default='RR_GAN_Data_Base', type=str, help='The name of database')
...
def save_Q_table( SINR, initial_real_reword, action, dronePos, episode, step, drone, name , collection_name, host='localhost', port=27017):
    myclient = pymongo.MongoClient(host='localhost', port=27017)
    mydb = myclient[name]
    dblist = myclient.list_database_names()
    data = {}
...
def direct_take_action(real_dronePos, dronePos):
    for i in range (np.shape(real_dronePos)[0]):
        for j in range((np.shape(real_dronePos)[1])):
            if abs(real_dronePos[i,j] - dronePos[i,j])>=5:
                if real_dronePos[i,j] < dronePos[i,j] and  real_dronePos[i,j]+10 < 100:
                    real_dronePos[i,j] += 10
                if real_dronePos[i,j] > dronePos[i,j] and  real_dronePos[i,j]-10 > 0:
                    real_dronePos[i,j] -= 10
    return real_dronePos

def choose_and_take_action(real_dronePos, dronePos, drone_num):
    if abs(real_dronePos[drone_num,0]-dronePos[drone_num,0]) > abs(real_dronePos[drone_num,1]-dronePos[drone_num,1]):
        if dronePos[drone_num,0] > real_dronePos[drone_num,0] and real_dronePos[drone_num, 0] + 10 < 100:
            real_dronePos[drone_num, 0] += 10
            action = 'east'
        elif dronePos[drone_num,0] < real_dronePos[drone_num,0] and real_dronePos[drone_num, 0] - 10 > 0:
            real_dronePos[drone_num, 0] -= 10
            action = 'west'
        else:
            action = 'stay'
    else:
        if dronePos[drone_num,1] > real_dronePos[drone_num,1] and real_dronePos[drone_num, 1] + 10 < 100:
            real_dronePos[drone_num, 1] += 10
            action = 'north'
        elif dronePos[drone_num,1] < real_dronePos[drone_num,1] and real_dronePos[drone_num, 1] - 10 > 0:
            real_dronePos[drone_num, 1] -= 10
            action = 'south'
        else:
            action = 'stay'

    return real_dronePos, action
...
def main(args):
    real_dronePos, _ = environment_setup()
    ...
    generator = UNet(1,2) # (channel, output class)
        ...
        for step in range(args.steps):
            ...
            if step % 50 == 0 :
                ...
                for num in range(args.stride):
                    ...
                    dronePos[1, 0] = np.array(np.where(fake_B.cpu().detach().numpy()[num, 1, :, :] == fake_B.cpu().detach().numpy()[num, 1, :, :].max()))[0, 0]
                    dronePos[1, 1] = np.array(np.where(fake_B.cpu().detach().numpy()[num, 1, :, :] == fake_B.cpu().detach().numpy()[num, 1, :, :].max()))[1, 0]

                    ideal_dronePos = np.ones((2, 3)) * 30
                    ideal_dronePos[0, 0] = np.array(np.where(
                        real_B.cpu().detach().numpy()[num, 0, :, :] == real_B.cpu().detach().numpy()[num, 0, :,
                                                                       :].max()))[0, 0]
                    ideal_dronePos[0, 1] = np.array(np.where(
                        real_B.cpu().detach().numpy()[num, 0, :, :] == real_B.cpu().detach().numpy()[num, 0, :,
                                                                       :].max()))[1, 0]
                    ideal_dronePos[1, 0] = np.array(np.where(
                        real_B.cpu().detach().numpy()[num, 1, :, :] == real_B.cpu().detach().numpy()[num, 1, :,
                                                                       :].max()))[0, 0]
                    ideal_dronePos[1, 1] = np.array(np.where(
                        real_B.cpu().detach().numpy()[num, 1, :, :] == real_B.cpu().detach().numpy()[num, 1, :,
                                                                       :].max()))[1, 0]

                    # print(dronePos)

                    _, _, ideal_max_reward_global = models.alloc_users(userPos, ideal_dronePos, args.fc, args.dAngle, args.N0,
                                                               args.BW,
                                                               args.Pt, args.connectThresh)
                    for k in range(args.numDrones):
                        real_dronePos, action = choose_and_take_action(real_dronePos, dronePos, k)
                        _, SINR, real_max_reward = models.alloc_users(userPos, real_dronePos, args.fc, args.dAngle,
                                                                      args.N0, args.BW,
                                                                      args.Pt, args.connectThresh)
                        save_Q_table(SINR, real_max_reward, action, real_dronePos, str(epoch), str(step), str(k), args.database_name, args.collection_name)

                    continues = True

                    # print(real_max_reward_global)
                    divider += 1
                    reward_max_pre += real_max_reward['total']
                    max_reward_real += ideal_max_reward_global['total']
                    if num >=6:
                        print('reward percentage',reward_max_pre*100/max_reward_real,'%','and',reward_max_pre/divider)
                        # print(location)
                        record += [reward_max_pre/divider]
                        # print(record)
